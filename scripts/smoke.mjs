...
if (seriesRecords(state).length !== 2800) throw new Error('series encyclopedia row count');
if (seriesDiscoverySummary(state).discovered < 1) throw new Error('initial series discovery missing');
if (SERIES_DEFINITIONS.length !== 2800) throw new Error(`series count ${SERIES_DEFINITIONS.length}`);
for (const manufacturer of MANUFACTURERS) { if (getSeriesForManufacturer(manufacturer.id).length !== 140) throw new Error(`series count for ${manufacturer.id}`); }
...
for (const profile of thirdGenProfiles) {
  ...
  const loreLength = ['concept','namingConcept','developmentBackground','engineeringNotes','trainingNotes'].reduce((sum, key) => sum + String(profile[key] ?? '').length, 0);
  if (loreLength < 650) throw new Error(`third-generation lore package too short: ${profile.id} ${loreLength}`);
  if ((profile.summary?.length ?? 0) > 170) throw new Error(`third-generation compact summary too long: ${profile.id}`);
  for (const field of ['namingConcept','developmentBackground','engineeringNotes','trainingNotes']) {
    if (!profile[field]) throw new Error(`third-generation lore missing ${field}: ${profile.id}`);
  }
...
for (const profile of sixthGenProfiles) {
  ...
  const loreLength = ['concept','namingConcept','developmentBackground','engineeringNotes','trainingNotes','weaponDoctrine'].reduce((sum, key) => sum + String(profile[key] ?? '').length, 0);
  if (loreLength < 480) throw new Error(`sixth-generation lore package too short: ${profile.id} ${loreLength}`);
  for (const field of ['namingConcept','developmentBackground','engineeringNotes','trainingNotes','weaponDoctrine']) {
    if (!profile[field]) throw new Error(`sixth-generation lore missing ${field}: ${profile.id}`);
  }
  if (profile.lineageGeneration !== 6 || profile.predecessorNumber < 81 || profile.predecessorNumber > 100) throw new Error(`sixth-generation lineage missing: ${profile.id}`);
  if (!profile.growthCurve?.label || !profile.customAptitude?.label || !profile.intrinsicTrait?.label) throw new Error(`sixth-generation identity missing: ${profile.id}`);
}
...
if (!migrated || migrated.version !== '3.8') throw new Error('migration version');
...
const thirdRefitInput = JSON.parse(JSON.stringify(secondRefitMigrated));
thirdRefitInput.version = '3.2';
const thirdRefitTarget = thirdRefitInput.roster[2] ?? thirdRefitInput.roster[0];
thirdRefitTarget.seriesId = 'kirishima-kaimon';
thirdRefitTarget.manufacturerId = 'kirishima';
thirdRefitTarget.seriesNumber = 41;
thirdRefitTarget.seriesNameKana = 'カイモン';
thirdRefitTarget.seriesTrendSummary = '旧第3世代説明';
delete thirdRefitTarget.seriesNamingConcept;
delete thirdRefitTarget.seriesDevelopmentBackground;
delete thirdRefitTarget.seriesEngineeringNotes;
delete thirdRefitTarget.seriesTrainingNotes;
delete thirdRefitTarget.seriesRefitGeneration;
delete thirdRefitTarget.seriesRefitVersion;
