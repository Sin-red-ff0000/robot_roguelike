...
if (seriesRecords(state).length !== 3200) throw new Error('series encyclopedia row count');
if (seriesDiscoverySummary(state).discovered < 1) throw new Error('initial series discovery missing');
if (SERIES_DEFINITIONS.length !== 3200) throw new Error(`series count ${SERIES_DEFINITIONS.length}`);
for (const manufacturer of MANUFACTURERS) { if (getSeriesForManufacturer(manufacturer.id).length !== 160) throw new Error(`series count for ${manufacturer.id}`); }
...
if (!migrated || migrated.version !== '3.9') throw new Error('migration version');
