import { SPECIAL_ABILITIES, NORMAL_POSITIVE_ABILITY_IDS, NEGATIVE_ABILITY_IDS } from '../src/data/specialAbilities.js?v=4.8';
import { STAT_GROUPS } from '../src/data/statDefinitions.js?v=4.8';
...
const themedIds = ['homeTechErgonomics','fieldMaintainability','expeditionSense','itineraryRebuild','sensualKinetics','poseControl','tactileCombatSense'];
const missingThemed = themedIds.filter((id) => !SPECIAL_ABILITIES[id]);
const result = {
  ok: abilities.length === 216 && badText.length === 0 && invalidBaseStat.length === 0 && new Set(weakNames).size === weakNames.length && missingThemed.length === 0,
