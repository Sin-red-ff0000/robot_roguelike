import { SERIES_DEFINITIONS, getSeriesForManufacturer } from '../src/data/seriesDefinitions.js?v=3.7';
import { seriesJapaneseDisplayIsValid } from '../src/data/seriesNameLocalization.js?v=3.7';
import { MANUFACTURERS } from '../src/data/manufacturers.js?v=3.7';
import { seriesDisplayName } from '../src/systems/displaySystem.js?v=3.7';

if (SERIES_DEFINITIONS.length !== 2400) throw new Error(`catalog size ${SERIES_DEFINITIONS.length}`);
for (const maker of MANUFACTURERS) {
  const list=getSeriesForManufacturer(maker.id);
  if (list.length !== 120) throw new Error(`${maker.id}: ${list.length} series`);
}
...
console.log(JSON.stringify({ok:true,series:SERIES_DEFINITIONS.length,perManufacturer:120,generations:6,invalidJapaneseLabels:invalid.length},null,2));
