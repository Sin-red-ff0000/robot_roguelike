...
import { MANUFACTURERS } from './data/manufacturers.js?v=4.0';
import { GROUP_KEYS, STAT_GROUPS, RESISTANCE_STATS } from './data/statDefinitions.js?v=4.0';
import { GROUP_GUIDE, STAT_GUIDE, WEAPON_AXIS_GUIDE, RESISTANCE_GUIDE, SYSTEM_GUIDE } from './data/statGuideDefinitions.js?v=4.0';
import { WEAPON_CATEGORIES, WEAPON_AXES } from './data/weaponDefinitions.js?v=4.0';
import { TOURNAMENTS, TOURNAMENT_IDS } from './data/tournamentDefinitions.js?v=4.0';
import { FACILITY_DEFINITIONS } from './data/facilityDefinitions.js?v=4.0';
import { createInitialState, advanceYear, migrateState } from './systems/gameState.js?v=4.0';
import { applyTrainingTurn, generateTrainingChoices, individualTrainingOptions } from './systems/trainingSystem.js?v=4.0';
import { simulateBattle } from './systems/battleSystem.js?v=4.0';
import { SPECIAL_ABILITIES } from './data/specialAbilities.js?v=4.0';
import { PART_RARITIES } from './data/partDefinitions.js?v=4.0';
import { adjustedCustomPartEffects, generateCustomPart, useCustomPart } from './systems/partSystem.js?v=4.0';
import { resolvePostTrainingEvent, tickTrainingModifiers } from './systems/eventSystem.js?v=4.0';
import { describeAbilityChange } from './systems/specialAbilitySystem.js?v=4.0';
import { analysisLevel, trainingChoiceCount, trainingLevelBias, updateFacilities } from './systems/facilitySystem.js?v=4.0';
import { battleWinTable, normalizeSettings, trainingTurnsForState } from './systems/settingsSystem.js?v=4.0';
import { manufacturerDisplayName, seriesDisplayName, robotDisplayName, robotFormalName } from './systems/displaySystem.js?v=4.0';
import { careerRecordSummary, manufacturerRecords, seriesRecords, seriesDiscoverySummary, isHallOfFame, toggleHallOfFame, updateHallMemo } from './systems/recordSystem.js?v=4.0';
import { downloadSave, readSaveFile } from './systems/saveSystem.js?v=4.0';
import { ensureTournamentYear, resolveTournamentMatch, tournamentEntry, tournamentMatchOptions, tournamentAvailableTurn } from './systems/tournamentSystem.js?v=4.0';
import { MANAGER_CONTEXT_LABELS, MANAGER_PERSONALITIES, MANAGER_TEMPLATE_TOKENS } from './data/managerDefinitions.js?v=4.0';
import { MANAGER_CUSTOM_MAX_LENGTH, MANAGER_CUSTOM_MAX_LINES, MANAGER_LINE_MODES, clearManagerCustomLines, loadManagerProfile, managerLine, parseCustomLines, renderManagerTemplate, resizeImageFile, saveManagerProfile, setManagerCustomLines, standardManagerLines } from './systems/managerSystem.js?v=4.0';
import { MANAGER_PRESET_LIMIT, applyManagerPreset, createManagerPreset, downloadManagerProfile, loadManagerPresets, readManagerProfileFile, removeManagerPreset, saveManagerPresets, upsertManagerPreset } from './systems/managerPresetSystem.js?v=4.0';
import { pwaInstallMessage, pwaInstallState, requestPwaInstall } from './systems/pwaSystem.js?v=4.0';
import {
  arrangeLineup,
  autoSelectLineup,
  ...
  updateLineupSlot,
} from './systems/teamMatchSystem.js?v=4.0';
...
function renderSeriesEncyclopedia() {
  ...
  const selector = `<div class="series-dex-toolbar series-dex-toolbar-advanced">
    ...
    <label>世代<select id="series-dex-generation"><option value="all" ${uiState.seriesDexGeneration==='all'?'selected':''}>全世代</option>${[1,2,3,4,5,6,7,8,9].map((g)=>`<option value="${g}" ${String(uiState.seriesDexGeneration)===String(g)?'selected':''}>第${g}世代 (#${(g-1)*20+1}～${g*20})</option>`).join('')}</select></label>
...
function renderSettingsPanel() {
  ...
  return `
    ...
    ${renderStatusGuide()}
    <p class="settings-note">シリーズは各メーカー180系列・合計3600系列を収録。図鑑には遭遇・加入・育成完了・殿堂の履歴が蓄積されます。マネージャーのマイカスタム設定はゲームセーブ本体とは別に保存されます。</p>
  `;
}
