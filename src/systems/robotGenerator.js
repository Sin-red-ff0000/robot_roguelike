...
import { getSeriesForManufacturer, resolveSeriesProfile } from '../data/seriesDefinitions.js?v=4.8';
import { GROUP_KEYS, RESISTANCE_STATS, STAT_GROUPS } from '../data/statDefinitions.js?v=4.8';
...
import { NEGATIVE_ABILITY_IDS, NORMAL_POSITIVE_ABILITY_IDS, SPECIAL_ABILITIES } from '../data/specialAbilities.js?v=4.8';
...
export function generateRobot({ year, cohortYear = 1, index = 1, manufacturerId = null, seriesId = null, weaponKey: forcedWeaponKey = null } = {}) {
  ...
  return {
    ...
    libidoBodyArchetypeId: seriesProfile?.libidoBodyArchetypeId ?? '',
    libidoBodyArchetype: seriesProfile?.libidoBodyArchetype ?? '',
    libidoBodyCombatRole: seriesProfile?.libidoBodyCombatRole ?? '',
    libidoBodyFeatureTags: [...(seriesProfile?.libidoBodyFeatureTags ?? [])],
    libidoBodyMechanicsScale: Number(seriesProfile?.libidoBodyMechanicsScale ?? 0),
